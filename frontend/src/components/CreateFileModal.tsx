...
export default function CreateFileModal({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!filename.trim()) {
      setError(mode === "file" ? "Filename is required" : "Folder name is required");
      return;
    }
    ...
    try {
      ...
      if (err instanceof Error) {
        if (err.message.includes("409") || err.message.includes("already exists")) {
          setError(mode === "file" ? "A file with this name already exists" : "A folder with this name already exists");
        ...
        <form onSubmit={handleSubmit}>
          <div className="mb-4">
            ...
            <input
              ...
              placeholder={mode === "file" ? "Enter filename (e.g., index.js, style.css)" : "Enter folder name"}
            />
            {error && (
            ...
            {currentPath && (
              <p className="mt-1 text-xs text-gray-500">
                Will be created in: <span className="font-mono text-blue-600">{currentPath}/</span>
              </p>
            )}
            {!currentPath && (
              <p className="mt-1 text-xs text-gray-500">
                Will be created in: <span className="font-mono text-blue-600">repository root</span>
              </p>
            )}
          </div>
          ...
          <div className="flex justify-end space-x-3">
            ...
            >
              ...
              <span>{isSubmitting ? "Creating..." : `Create ${mode === "file" ? "File" : "Folder"}`}</span>
            </button>
          </div>
        </form>
      </div>
