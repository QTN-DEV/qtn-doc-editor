        ...
        for i, line in enumerate(lines):
            ...
            if stripped_line.startswith("class "):
                ...
                if class_name_end != -1:
                    ...
                    current_class = (
                        stripped_line[class_name_start:].strip().replace(":", "")
                    )
            ...
            if stripped_line.startswith("def ") or is_async:
                ...
                if "->" in stripped_line:
                    ...
                    if return_type:
                        return_type = [
                            x.strip() for x in return_type.split(",") if x.strip()
                        ]
                ...
                for param in params_str.split(","):
                    param = param.strip()
                    if param:
                        param_with_default_value = param.split("=")
                        param_parts = param_with_default_value[0].split(":")
                        ...
                        if len(param_with_default_value) > 1:
                            input_schema[param_name]["default"] = (
                                param_with_default_value[1].strip()
                            )
                        input_schema[param_name]["required"] = (
                            len(param_with_default_value) == 1
                        )
                ...
                for j in range(i + 1, len(lines)):
                    ...
                    if doc_line.startswith('"""') or doc_line.startswith("'''"):
                        ...
                        if (
                            doc_line.count(doc_start_quote) == 2
                            ...
                            docs = (
                                full_doc_string.strip(doc_start_quote)
                                .strip(doc_end_quote)
                                .strip()
                            )
                    ...
                    if (
                        doc_line.startswith("def ")
                        or doc_line.startswith("class ")
                        or not doc_line.startswith(" ")
                    ):
                        break
